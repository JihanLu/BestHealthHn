function selectTitle1(){
		  ...
		  if(title==""){
			  ...
			  window.location.href="/admin/literature/selectTitle.html?title="+title+"";
		  }
	}
//批量删除
 function datadel(){
	...
	for(k in boxes){
 	...
 	if (Checkbox){
 		 layer.confirm('您确认要删除选中的内容吗？', {
 		    ...
 		    $.ajax({
 		        url:"/admin/literature/delectAll.html",
 		        dataType: "json",
 		        type: "post",
 ...
 function article_edit(id){
 	window.location.href="/admin/literature/select.html?id="+id+"";
 }
 ...
 function activity_del(obj,id){
 	layer.confirm('确认要删除吗？',function(index){
 		$.ajax({
 			type: 'POST',
 			url: '/admin/literature/delect.html',
 			dataType: 'json',
 			  data: {"id": id},
 			success: function(data){
 				if(data.del==0){
