function selectTitle1(){
		  ...
		  if(title==""){
			  ...
			  window.location.href="/admin/replyOne/selectTitle.html?title="+title;
		  }
	}
//批量删除
 function datadel(){
	...
	for(k in boxes){
 	...
 	if (Checkbox){
 		 layer.confirm('您确认要删除选中的内容吗？', {
 		    ...
 		    $.ajax({
 		        url:"/admin/replyOne/delectAll.html",
 		        dataType: "json",
 		        type: "post",
 		        ...
 		        success: function (data) {
 		        	...
 		        	if(idDataLen==0){
 		        		...
 		        		//alert(pageNow)
 		        		var title = $("#aaa").val();
 		        		window.location.href="/admin/replyOne/list1.html?title="+title+"&pageNow="+pageNow;
 		        	}*/
 ...
 function activity_del(obj,id){
 	layer.confirm('确认要删除吗？',function(index){
 		$.ajax({
 			type: 'POST',
 			url: '/admin/replyOne/delect.html',
 			dataType: 'json',
 			  data: {"id": id},
 			success: function(data){
 				if(data.del==0){
