function selectTitle1(){
		  ...
		  if(title==""){
			  ...
			  window.location.href="/admin/topic/selectTitle.html?title="+title;
		  }
	}
//批量删除
 function datadel(){
	...
	for(k in boxes){
 	...
 	if (Checkbox){
 		 layer.confirm('您确认要删除选中的内容吗？', {
 		    ...
 		    $.ajax({
 		        url:"/admin/topic/delectAll.html",
 		        dataType: "json",
 		        type: "post",
 		        ...
 		        success: function (data) {
 		        	...
 		        	if(idDataLen==0){
 		        		...
 		        		alert(pageNow)
 		        		var title = $("#aaa").val();
 		        		window.location.href="/admin/topic/list.html?title="+title+"&pageNow="+pageNow;
 		        	}*/
