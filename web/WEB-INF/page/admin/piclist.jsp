...
<div>
    ...
    <div style="display:block;width:50%;position:absolute;left:5%;top:50px">
        <div class="table-responsive" style="width:90%;margin-left:0px;overflow:hidden">
            <form id="fileDataForm" action="<%=basePath %>admin/picture/upload" enctype="multipart/form-data" method="post">
                <table class="table table-striped">
                    ...
                    <tr>
                        ...
                        <td>
                            <div>
                                <input type="file" name="targetFile1" id="targetFile1" onchange="checkFileExt(this.value)">
                        ...
                        <td><button type="button" class="btn btn-primary"  id="uploadButton">上传批复文件扫描件</button></td>
                    </tr>
                    </tbody>
                </table>
            </form>
...
<script>
    ...
    function initForm(table){
        ...
        $.ajax({
            type:"POST",
            url:"<%=basePath%>admin/picture/dataList",
            success:function(data){
                $(data).each(function(index,value){
                    html+="<tr>"+
                        "<input type='hidden' name='fileName' value='"+value.fileName+"'/>"+
                        "<td>"+(index+1)+".</td>"+
                        "<td><a href='<%=basePath%>admin/picture/download?fileName="+value.fileName+"'>"+value.fileName+"</a></td>"+
    ...
    function removeFile(obj){
        ...
        if(flag){
            $.ajax({
                type:"POST",
                url:"<%=basePath%>admin/picture/remove",
                dataType:"json",
    ...
    {
        var flag = false; //状态
        var arr = ["jpg","png","gif","bmp","tiff","psd","pcd","pcx"];
        //取出上传文件的扩展名
        ...
        if(!flag)
        {
            document.write("文件名不合法,请上传图片格式的文件");
        }
    }
