...
<body>
	...
	<div class="page-container">
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			...
			<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>
					批量删除</a> 
			<a class="btn btn-primary radius" data-title="添加产品"
				href="${pageContext.request.contextPath}/admin/recommend/insertjsp.html">
				<i class="Hui-iconfont">&#xe600;</i> 添加产品</a>
				<input type="text" value="${title}" id="aaa" name="selectTitle" placeholder=" 产品名称"
				...
				class="r">共有数据：<strong>${recommendlist.size()}</strong> 条
			</span> --%>
		</div>
		<div class="mt-20">
			<table
				...
				<tbody id="idData">
					<c:forEach items="${recommendlist}" var="recommend">
						<tr class="text-c">
							<td><input type="checkbox" value="${recommend.id}" name="id[]" onclick="check()"></td>
							<%-- <td>${recommend.id}</td> --%>
							<td>${recommend.title}</td>
							<td>${recommend.author}</td>
							<td>${recommend.sources}</td>
							<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${recommend.time}"/></td>


							<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5"
								href="${pageContext.request.contextPath}/admin/recommend/updateView.html?id=${recommend.id}" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration: none" class="ml-5" onClick="activity_del(this,'${recommend.id}')" href="javascript:;"
	...
	<form action="${pageContext.request.contextPath}/admin/recommend/selectTitle.html" method="post" id="pageform"></form>
	...
	<script type="text/javascript"
		...
		<script type="text/javascript"
		src="${pageContext.request.contextPath}/static/admin/lib/recommedpaging.js"></script>
	<script type="text/javascript">
