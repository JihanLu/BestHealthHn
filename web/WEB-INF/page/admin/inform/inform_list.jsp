...
<body>
	<nav class="breadcrumb">
		...
		通告管理 <span class="c-gray en">&gt;</span> 通告列表 <a
	...
	<div class="page-container">
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			...
			<a class="btn btn-primary radius" data-title="添加通告"
				href="${pageContext.request.contextPath}/admin/inform/insertjsp.html">
				<i class="Hui-iconfont">&#xe600;</i> 添加通告</a>
				<input type="text" value="${title}" id="aaa" name="selectTitle" placeholder="请输入通告标题"
				...
				class="r">共有数据：<strong>${informlist.size()}</strong> 条
			</span> --%>
		</div>
		<div class="mt-20">
			<table
				...
				<tbody id="idData">
					<c:forEach items="${informlist}" var="inform">
						<tr class="text-c">
							<td><input type="checkbox" value="${inform.id}" name="id[]" onclick="check()"></td>
						<%-- 	<td>${inform.id}</td> --%>
							<td>${inform.title}</td>
							<td>${inform.author}</td>
							<td>${inform.sources}</td>
							<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${inform.time}"/></td>


							<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5"
								href="${pageContext.request.contextPath}/admin/inform/updateView.html?id=${inform.id}" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration: none" class="ml-5" onClick="activity_del(this,'${inform.id}')" href="javascript:;"
	...
	<form action="${pageContext.request.contextPath}/admin/inform/selectTitle.html" method="post" id="pageform"></form>
	...
	<script type="text/javascript"
		...
		<script type="text/javascript"
		src="${pageContext.request.contextPath}/static/admin/lib/informpaging.js"></script>
	<script type="text/javascript">
