...
<body>
	...
	<div class="page-container">
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			...
			<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>
					批量删除</a> 
			<a class="btn btn-primary radius" data-title="添加资讯新闻"
				href="${pageContext.request.contextPath}/admin/news/insertjsp.html">
				<i class="Hui-iconfont">&#xe600;</i> 添加资讯新闻</a>
				<input type="text" value="${title}" id="aaa" name="selectTitle" placeholder="请输入资讯新闻标题"
				...
				class="r">共有数据：<strong>${newslist.size()}</strong> 条
			</span> --%>
		</div>
		<div class="mt-20">
			<table
				...
				<tbody id="idData">
					<c:forEach items="${newslist}" var="news">
						<tr class="text-c">
							<td><input type="checkbox" value="${news.id}" name="id[]" onclick="check()"></td>
							<%-- <td>${news.id}</td> --%>
							<td>${news.title}</td>
							<td>${news.author}</td>
							<td><c:choose><c:when test="${news.topid==0}">否</c:when>
							<c:otherwise><font color="red">是</font></c:otherwise></c:choose></td>
							<td>${news.sources}</td>
							<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${news.time}"/></td>


							<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5"
								href="${pageContext.request.contextPath}/admin/news/updateView.html?id=${news.id}" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration: none" class="ml-5" onClick="news_del(this,'${news.id}')" href="javascript:;"
	...
	<form action="${pageContext.request.contextPath}/admin/news/selectTitle.html" method="post" id="pageform"></form>
	...
	<script type="text/javascript"
		...
		<script type="text/javascript"
		src="${pageContext.request.contextPath}/static/admin/lib/newspaging.js"></script>
	<script type="text/javascript">
