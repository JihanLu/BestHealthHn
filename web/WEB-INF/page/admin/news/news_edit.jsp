...
<article class="page-container">
	<form class="form form-horizontal" id="form-article-add" enctype="multipart/form-data" action="${pageContext.request.contextPath}/admin/news/update.html" method="post">
		<div class="row cl">
		...
		<label class="form-label col-xs-4 col-sm-2">置顶：</label>
			<div class="formControls col-xs-8 col-sm-9"> 
				<select name="topid" id="topid" class="required"
					...
					<c:if test="${news.topid==0}">
						<option value="0">否</option>
						<option value="1">是</option>
					</c:if>
					<c:if test="${news.topid==1}">
						<option value="1">是</option>
						<option value="0">否</option>
					</c:if></select><br>
			</div>
